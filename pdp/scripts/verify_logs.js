...
function validateSchema(pdpAudit) {
  ...
  if (Number.isNaN(Date.parse(pdpAudit.timestamp))) return false;
...
function stable(obj) {
  ...
  return Object.keys(obj).sort().reduce((out, k) => {
...
function findJws(obj) {
  ...
  if (obj.message && typeof obj.message === 'string') {
    const maybe = tryParseJson(obj.message);
...
function findSignedPayload(obj) {
  ...
  if (obj.message && typeof obj.message === 'string') {
    const m = tryParseJson(obj.message);
    if (m && m.signedPayload) return m.signedPayload;
  }
  for (const k of Object.keys(obj)) {
...
function extractPdpAudit(parsed) {
  if (parsed.pdpAudit) return parsed.pdpAudit;
  if (parsed.message && typeof parsed.message === 'object' && parsed.message.pdpAudit) {
    return parsed.message.pdpAudit;
  }
  if (parsed.message && typeof parsed.message === 'string') {
    const m = tryParseJson(parsed.message);
    if (m && m.pdpAudit) return m.pdpAudit;
  }
  return null;
}
  ...
  return new Promise((resolve, reject) => {
    ...
    rl.on('line', (line) => {
      ...
      try {
        ...
        local.jsonParseErrors += 1;
        console.error(`${file}:${local.totalLines} JSON parse error: ${err.message}`);
        return;
      }

      const pdpAudit = extractPdpAudit(parsed);
      if (!pdpAudit) {
        local.missingPdpAudit += 1;
        return;
      }

      if (!validateSchema(pdpAudit)) {
        local.schemaInvalid += 1;
      ...
      try {
        ...
        const payloadStr = decoded && decoded.payload ? decoded.payload : null;
        ...
        local.signedInvalid += 1;
        console.error(`${file}:${local.totalLines} verification error: ${err.message || err}`);
      }
    });
...
(async function main() {
  for (const f of files) {
    ...
    const stats = await processFile(f); // eslint-disable-line no-await-in-loop
    Object.keys(aggregate).forEach((k) => { aggregate[k] += stats[k] || 0; });
