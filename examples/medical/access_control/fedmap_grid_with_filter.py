...
class FedMAPWithFilter(Strategy):
    ...
    This implementation delegates authorization to an external access validator
    (a PEP-backed validator) to decide node participation.

    Parameters
    ----------
    access_validator : object
        External access control validator with methods:
        - is_allowed_full_training(node_id) -> tuple[bool, str]
        - is_allowed_to_evaluate(node_id) -> tuple[bool, str]
    federation : str
        Name of the federation for logging purposes.
    ...
    def __init__(
        ...
        log(INFO, "FedMAPWithFilter initialized for federation '%s'", federation)
    ...
    def configure_train(
        ...
        """Configure the next round of federated training — exclude nodes denied by the PEP."""
        if self.fraction_train == 0.0:
            return []

        log(INFO, "=" * 60)
        log(INFO, "[Round %d] Configuring Training Tasks (FedMAP)", server_round)
        log(INFO, "=" * 60)
        ...
        # Create messages for all sampled nodes
        messages_list = list(self._construct_messages(record, node_ids, MessageType.TRAIN))
        
        if not messages_list:
            return []

        # Filter messages based on access control
        eligible_messages = []
        excluded_nodes_info = []
    ...
    def aggregate_train(
        ...
        """Aggregate ArrayRecords and MetricRecords — filter replies using PEP."""
        log(INFO, "=" * 60)
        log(INFO, "[Round %d] Training Aggregation (FedMAP)", server_round)
        log(INFO, "=" * 60)
        ...
        # Filter replies based on access control
        for message in replies_list:
            if message.has_error():
                log(WARNING, "  ❌ Node %d: Message has error - %s", 
                    message.metadata.src_node_id, message.error.reason)
                continue
        ...
        # Proceed with FedMAP aggregation logic using only eligible replies
        valid_replies, _ = self._check_and_log_replies(eligible_replies, is_train=True)
    ...
    # =========================================================================
    # Evaluation Configuration and Aggregation with Access Control
    # =========================================================================
            
    def configure_evaluate(
        self, server_round: int, arrays: ArrayRecord, config: ConfigRecord, grid: Grid
    ) -> Iterable[Message]:
        """Configure the next round of federated evaluation — exclude nodes denied by the PEP."""
        if self.fraction_evaluate == 0.0:
            return []

        log(INFO, "\n📊 [Round %d] Configuring Evaluation Tasks (FedMAP)", server_round)
        ...
        messages_list = list(self._construct_messages(record, node_ids, MessageType.EVALUATE))
    ...
    def aggregate_evaluate(
        ...
        log(INFO, "\n📊 [Round %d] Evaluation Aggregation (FedMAP)", server_round)
